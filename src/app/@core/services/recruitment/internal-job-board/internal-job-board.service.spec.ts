...
describe('InternalJobBoardService', () => {
  let service: InternalJobBoardService;
  let httpMock: HttpTestingController;
  const API_URL = `${BASE_URL_Recruitment}/internal-job-board`;

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [HttpClientTestingModule],
      providers: [InternalJobBoardService],
    });

    service = TestBed.inject(InternalJobBoardService);
  ...
  it('should submit an application via POST /job-postings/{id}/apply', () => {
    ...
    req.flush({ hasError: false, decentMessage: '', content: { jobApplicationId: 1, jobPostingId: 5, candidateName: 'John Doe', applicationStatus: 'Submitted', source: 'Internal' } });
  });

  it('should reference the correct enum values for reference data', () => {
    expect(EmploymentTypeEnum.FullTime).toBe('FullTime');
    expect(CircularTypeEnum.InternalOnly).toBe('InternalOnly');
  });
});
